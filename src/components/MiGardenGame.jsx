...
export default function MiGardenGame(){
 ...
 const dash=useRef(0),shake=useRef(0),particles=useRef([]),checkpoint=useRef({x:180,y:1080}),anim=useRef({dir:"down",moving:false,step:0}),wind=useRef(0);
 const [open,setOpen]=useState(false),[found,setFound]=useState(()=>+localStorage.getItem("mi-v4-f")||0),[mem,setMem]=useState(()=>+localStorage.getItem("mi-v4-m")||0),[lives,setLives]=useState(3),[msg,setMsg]=useState("Esse jardim parece ter perdido as cores. Talvez você consiga encontrá-las."),[near,setNear]=useState(false),[done,setDone]=useState(()=>localStorage.getItem("mi-v4-d")==="1");
 ...
 if(inv.current>0)inv.current-=dt;sn.current.forEach((z,i)=>{
 ...
 x.save();x.translate(q.x,q.y+bob);if(an.dir==="left")x.scale(-1,1);x.globalAlpha=inv.current>0&&Math.floor(inv.current/5)%2?.35:1;
 ...
 if(catsSolved>secret&&secret<SECRET_GATES.length){let g=SECRET_GATES[secret];if(Math.hypot(q.x-g[0],q.y-g[1])<75)n=true}
 ...
 if(catsSolved>secret&&secret<SECRET_GATES.length){let g=SECRET_GATES[secret];if(Math.hypot(q.x-g[0],q.y-g[1])<80){setSecret(v=>v+1);setXp(v=>v+50);setToast("ÁREA SECRETA DESCOBERTA +50 XP 🔐");setMsg(`Você encontrou: ${g[2]}. Nem todos os caminhos aparecem no mapa.`);if(secret===2){q.x=2040;q.y=1370;}return}}
if(mem<8){let a=memories[mem];if(Math.hypot(q.x-a[0],q.y-a[1])<60){setMem(v=>v+1);setXp(v=>v+15);setToast("+15 XP • MEMÓRIA ENCONTRADA 📷");particles.current.push(...Array.from({length:10},(_,i)=>({x:q.x+(i-5)*4,y:q.y,life:45,e:"✨"})));setMsg(`Memória ${mem+1}/8 encontrada. Algumas coisas merecem um lugar onde não possam se perder. 📷`);return}}if(found<8){let a=fragments[found];if(Math.hypot(q.x-a[0],q.y-a[1])<70){setFound(v=>v+1);setXp(v=>v+35);setQuest(v=>v+1);setToast("+35 XP • FRAGMENTO RECUPERADO ✨");particles.current.push(...Array.from({length:14},(_,i)=>({x:q.x+(i-7)*3,y:q.y,life:50,e:i%3?"✨":"🌸"})));setMsg(a[3]);return}}if(found>=8&&!done&&Math.hypot(q.x-890,q.y-180)<110){setDone(true);localStorage.setItem("mi-v4-d","1");unlockSecret("garden-complete");setMsg("No começo esse lugar estava quase vazio. Você encontrou as cores, as flores, as memórias e até aquele gato. Mas a parte que estava faltando era você. 🤍  22.08.2026")}}
 ...
 function stick(e){let el=e.currentTarget,r=el.getBoundingClientRect();let mv=ev=>{let t=ev.touches?.[0]||ev;joy.current={x:clamp((t.clientX-r.left-r.width/2)/(r.width*.35),-1,1),y:clamp((t.clientY-r.top-r.height/2)/(r.height*.35),-1,1)}};mv(e);let end=()=>{joy.current={x:0,y:0};removeEventListener("pointermove",mv);removeEventListener("pointerup",end)};addEventListener("pointermove",mv);addEventListener("pointerup",end)}
