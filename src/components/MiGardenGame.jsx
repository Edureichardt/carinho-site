...
export default function MiGardenGame(){
 ...
 arrowLoot.forEach(([ax,ay])=>{if(bow&&Math.hypot(q.x-ax,q.y-ay)<38&&arrows<8){setArrows(v=>Math.min(8,v+1));setToast("FLECHA ENCONTRADA +1 🏹")}})
 ...
 // loot e casa segura
 if(bow){[[420,1480],[1320,330],[2110,760],[2510,1320]].forEach(([ax,ay])=>{x.font="20px serif";x.fillText("🏹",ax,ay)})}
 ...
 if(items.sword===0&&Math.hypot(q.x-2380,q.y-1510)<42){setItems(v=>({...v,sword:1}));setToast("ESPADA DE LUZ EQUIPADA ⚔️");setMsg("Agora as aranhas recuam quando chegam perto de você.")}
 sn.current.forEach((z,i)=>{
   ...
   if(items.sword>0&&d<105){const a=Math.atan2(z.y-q.y,z.x-q.x);z.x+=Math.cos(a)*3.2*dt;z.y+=Math.sin(a)*3.2*dt;z.alert=false;return}
 ...
 if(companion==="edu"&&bow&&arrows>0&&!npcOpen&&!secretScene&&now-eduShot.current>8000){let best=null,bd=360;sn.current.forEach(z=>{let d=Math.hypot(q.x-z.x,q.y-z.y);if(z.stun<=0&&d<bd){best=z;bd=d}});if(best){best.slow=240;setArrows(v=>Math.max(0,v-1));eduShot.current=now;setToast("EDU ACERTOU UMA FLECHA 🏹")}}
 ...
 else {x.fillStyle="#172033";x.beginPath();x.arc(ca.x,ca.y-9,10,0,7);x.fill();x.fillStyle="#e8b899";x.beginPath();x.arc(ca.x,ca.y-17,7,0,7);x.fill();x.fillStyle="#334155";x.fillRect(ca.x-8,ca.y-8,16,19);x.fillStyle="#60a5fa";x.font="10px sans-serif";x.fillText("♥",ca.x-3,ca.y+4);x.font="9px sans-serif";x.fillStyle="rgba(15,23,42,.75)";x.fillText("Edu",ca.x-7,ca.y+22)}
...
if(mem<8){let a=memories[mem];if(Math.hypot(q.x-a[0],q.y-a[1])<60){setMem(v=>v+1);setXp(v=>v+15);setToast("+15 XP • MEMÓRIA ENCONTRADA 📷");particles.current.push(...Array.from({length:10},(_,i)=>({x:q.x+(i-5)*4,y:q.y,life:45,e:"✨"})));setMsg(`Memória ${mem+1}/8 encontrada. Algumas coisas merecem um lugar onde não possam se perder. 📷`);return}}if(found<8){let a=fragments[found];if(Math.hypot(q.x-a[0],q.y-a[1])<70){setFound(v=>v+1);setXp(v=>v+35);setQuest(v=>v+1);setToast("+35 XP • FRAGMENTO RECUPERADO ✨");particles.current.push(...Array.from({length:14},(_,i)=>({x:q.x+(i-7)*3,y:q.y,life:50,e:i%3?"✨":"🌸"})));setMsg(a[3]);return}}if(found>=8&&!done&&Math.hypot(q.x-890,q.y-180)<110){setDone(true);localStorage.setItem("mi-v4-d","1");unlockSecret("garden-complete");setMsg("No começo esse lugar estava quase vazio. Você encontrou as cores, as flores, as memórias e até aquele gato. Mas a parte que estava faltando era você. 🤍  22.08.2026")}}
 ...
 {bossOpen&&<div className="fixed inset-0 z-[180] grid place-items-center bg-slate-950 p-5 text-center text-white"><div className="w-full max-w-sm"><div className="text-7xl">{bossHp>6?"🕷️":bossHp>3?"🕷️🕸️":"🕷️🔥"}</div><h3 className="mt-3 text-2xl font-black">A Guardiã das Teias</h3><div className="mt-2 text-[10px] font-black tracking-widest text-white/60">{bossHp>6?"FASE 1 • DESPERTAR":bossHp>3?"FASE 2 • TEIAS":"FASE 3 • FÚRIA"}</div><p className="mt-2 text-xs text-white/60">{bossHp>6?"Ela ainda está estudando seus movimentos.":bossHp>3?"As teias ficam mais densas. Flechas são muito úteis agora.":"Última fase. A Guardiã está furiosa — termine a luta!"}</p><div className="mx-auto mt-5 h-3 max-w-xs overflow-hidden rounded-full bg-white/10"><div className="h-full bg-white transition-all" style={{width:`${bossHp/9*100}%`}}/></div><p className="mt-2 text-xs">{bossHp}/9 ❤️</p><div className="mt-5 grid grid-cols-2 gap-3"><button onClick={()=>{bossAttack();setTimeout(bossCounter,250)}} className="min-h-14 rounded-xl bg-white font-black text-slate-950">⚔️ ATACAR</button><button onClick={()=>{if(arrows>0){setArrows(v=>v-1);setBossHp(v=>{let n=Math.max(0,v-1);if(n===0){setEnding(true);setBossOpen(false);setXp(x=>x+500)}return n});setToast("FLECHA ACERTOU 🏹");setTimeout(bossCounter,250)}}} disabled={!bow||arrows<=0} className="min-h-14 rounded-xl border border-white/30 bg-white/10 font-black disabled:opacity-30">🏹 FLECHA ({arrows})</button></div><button onClick={()=>setBossOpen(false)} className="mt-5 text-xs opacity-60">recuar para o parque</button></div></div>}
 {ending&&<div className="fixed inset-0 z-[200] grid place-items-center bg-gradient-to-b from-blue-950 to-slate-950 p-6 text-center text-white"><div className="max-w-sm"><div className="text-6xl">🌸💙✨</div><h2 className="mt-5 text-3xl font-black">Você devolveu as cores ao jardim.</h2><p className="mt-4 text-sm leading-7 text-blue-100">Mas o melhor segredo nunca esteve escondido no mapa. Era ter você caminhando por ele.</p><p className="mt-5 text-xl font-black">Eu te amo, Mi. 🤍</p><button onClick={()=>{setEnding(false);setBossHp(9)}} className="mt-8 min-h-14 w-full rounded-2xl bg-white font-black text-blue-950">CONTINUAR EXPLORANDO</button></div></div>}
